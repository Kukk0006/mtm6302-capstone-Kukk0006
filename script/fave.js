...
function buildGallery () {
  ...
  for (let i = 0; i < favs.length; i++) {
    html.push(`<span class="card grid-item">
      <img src="${favs[i].url}" alt="${favs[i].title}" data-source="${favs[i].title}" data-date="${favs[i].date}" data-index="${i}" data-large="${favs[i].hdurl}" class="galleryImg imgRestrict pb-2"> <br>
...
function clickHandler (e) {
  ...
  if (e.target.classList.contains('galleryImg')) {
    ...
    <div class="caption"><h3>${e.target.dataset.source}</h3></div>
    `;
    $modalHD.classList.add('show');
    // swal.fire({
    //   icon: "warning",
    //   title: "Oh noes",
